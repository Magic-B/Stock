...
export const useStockStore = defineStore("stock", {
  ...
  actions: {
    ...
    addTo (product: Product, where: ProductsType): void {
      const isExist = this[where].find((item: Product) => item.id === product.id)
      
      if (!isExist) this[where].push(product)
    },
    removeFrom (product: Product, where: ProductsType): void {
      this[where] = this[where].filter((item: Product) => item.id !== product.id)
    }
  },
  ...
  getters: {
    filterBy: (state) => {
      return (sortBy: ProductsType) => {
        if (state.selected) {
          return state[sortBy].filter((item: Product) => state.selected === item.type)
        }
        return state[sortBy]
      }
    },
    searchBy () {
      return (sortBy: ProductsType) => this.filterBy(sortBy).filter((product: Product) => product.title.toLowerCase().indexOf(this.searchInput.toLowerCase()) !== -1)
    }
  }
});
